...
ARTICLE="docs/insights/insight-003-the-implementation-gap.md"
INDEX="docs/insights/index.md"
NAV="mkdocs.yml"
MENU="docs/javascripts/orsm-megamenu.js"

echo
echo "========================================"
echo " ORSM Insight 003"
echo " The Implementation Gap"
echo "========================================"
echo
...
BACKUP=".orsm-backups/insight-003-$STAMP"
...
[ -f "$ARTICLE" ] && cp "$ARTICLE" "$BACKUP/"

echo "✓ Backup created: $BACKUP"

# ------------------------------------------------------------
# 3. Create Insight 003
# ------------------------------------------------------------
...
p = Path("docs/insights/index.md")
text = p.read_text()

if "insight-003-the-implementation-gap.md" not in text:

    block = """### Insight 003 — The Implementation Gap

**Your threat model may not be wrong. Your architecture may have moved on.**

Threat knowledge may remain valid while architecture, trust relationships, dependencies, control effectiveness and consequence change around it.
...
target = "insights/insight-003-the-implementation-gap.md"

if not any(target in x for x in lines):

    for i, line in enumerate(lines):

        if "insights/control-fatigue.md" in line:

            indent = line[:len(line) - len(line.lstrip())]

            lines.insert(
                i + 1,
                f'{indent}- "Insight 003 — The Implementation Gap": {target}'
            )

            break
    else:
        raise SystemExit(
            "ERROR: Could not find Insight 002 in mkdocs.yml"
        )
...
# ------------------------------------------------------------
# 6. Update mega-menu Latest
# ------------------------------------------------------------

python3 <<'PY'
from pathlib import Path

p = Path("docs/javascripts/orsm-megamenu.js")
text = p.read_text()

old_url = "/insights/control-fatigue/"
new_url = "/insights/insight-003-the-implementation-gap/"

if new_url not in text:

    if old_url not in text:
        raise SystemExit(
            "ERROR: Could not find Insight 002 mega-menu link"
        )
    ...
    text = text.replace(
      ...
      Control Fatigue""",
        """The Implementation Gap
      <br>
      Your threat model may not be wrong.""",
        1
    )

    p.write_text(text)

print("✓ Mega-menu updated")
PY
...
echo
echo "Checking Insight 003..."

test -s "$ARTICLE"

grep -q "INSIGHT 003" "$ARTICLE"
grep -q "insight-003-the-implementation-gap.md" "$INDEX"
grep -q "insight-003-the-implementation-gap.md" "$NAV"
grep -q "insight-003-the-implementation-gap" "$MENU"

echo "✓ All four locations verified"
...
echo
echo "========================================"
echo " INSIGHT 003 IS PREPARED LOCALLY"
...
echo
echo "If everything above looks good:"
echo
echo "  git add docs/insights/insight-003-the-implementation-gap.md \\"
echo "          docs/insights/index.md \\"
echo "          mkdocs.yml \\"
echo "          docs/javascripts/orsm-megamenu.js"
echo
echo '  git commit -m "Publish ORSM Insight 003 The Implementation Gap"'
